...
def compute_vol_history_features(df, config_features=None):
    ...
    if any(config_features.get(k) for k in ["days_since_iv_year_high", "days_since_iv_year_low", "days_since_hv_year_high", "days_since_hv_year_low"]):
        date_ts = pd.to_datetime(pd.Series(df["date"].values))

    if config_features.get("days_since_iv_year_high"):
        iv_high_ts = pd.to_datetime(pd.Series(df["iv_year_high_date"].values))
    ...
    if config_features.get("days_since_iv_year_low"):
        iv_low_ts = pd.to_datetime(pd.Series(df["iv_year_low_date"].values))
    ...
    if config_features.get("days_since_hv_year_high"):
        hv_high_ts = pd.to_datetime(pd.Series(df["hv_year_high_date"].values))
    ...
    if config_features.get("days_since_hv_year_low"):
        hv_low_ts = pd.to_datetime(pd.Series(df["hv_year_low_date"].values))
